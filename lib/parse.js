 ...
 * @returns {string}
 */
// NOTE: slugify / splitBullets / splitSpecs are sheet-transcription-time helpers.
// data/products.json already ships pre-computed slugs and parsed arrays, so the
// runtime generator (build/verify/rewire) does not call them — they're retained
// (and tested) for regenerating product data from the sheet. escapeHtml and
// formatPrice ARE used by the renderers.
 ...
 * Bullets may be separated by newlines (each line prefixed with `-`, `*`, or `•`)
 * or, within a single line, by inline `* ` markers. The marker itself is stripped.
 * @param {string} raw
 * @returns {string[]}
 */
export function splitBullets(raw) {
  ...
  return String(raw)
    ...
    .map((item) => item.replace(/^[*•-]\s*/, '').trim()) // strip a leading -, *, or • marker
    .filter(Boolean);
}
